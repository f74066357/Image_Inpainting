model = dict(
    ...
    stage2_loss_type=('loss_l1_hole', 'loss_l1_valid', 'loss_gan','loss_composed_percep','loss_out_percep'),
    loss_gan=dict(
        type='GANLoss',
        gan_type='hinge',
        loss_weight=0.1,
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file='data/new_imglist_train.txt',
        data_prefix=data_root,
        pipeline=train_pipeline,
        test_mode=False),
    val=dict(
        type=dataset_type,
        ann_file='data/new_imglist_test.txt',
        data_prefix=data_root,
...
optimizers = dict(
    generator=dict(type='Adam', lr=0.0005), disc=dict(type='Adam', lr=0.0005))
...
log_level = 'INFO'
work_dir = './work_dirs/learningrate'
load_from = './models/deepfillv2_256x256_8x2_places.pth'
resume_from = None
workflow = [('train', 10000)]
exp_name = 'deepfillv2_256x256_8x2_places_load'
